        ...
        <div class="space-y-20">
            @foreach($categories as $cat)
                <div class="relative group">
                    ...
                    <div class="relative glass-card rounded-3xl p-8 lg:p-12 border border-slate-800/80 bg-slate-950/40 backdrop-blur-md transition-all duration-500 overflow-hidden {{ $cat['bgGlow'] }}">
                        ...
                        <div class="absolute top-0 right-10 px-4 py-1.5 bg-slate-900 border-x border-b border-slate-800/60 rounded-b-xl flex items-center gap-2">
                            <span class="relative flex h-2 w-2">
                                <span class="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75" style="background-color: {{ $cat['accent'] }}"></span>
                                <span class="relative inline-flex rounded-full h-2 w-2" style="background-color: {{ $cat['accent'] }}"></span>
                            </span>
                            <span class="text-[10px] font-mono text-slate-500 tracking-wider uppercase">{{ $cat['badge'] }}</span>
                        </div>
                        ...
                        <div class="mb-12 border-b border-slate-800/50 pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
                            <div>
                                <span class="text-xs font-mono tracking-widest uppercase" style="color: {{ $cat['accent'] }}">{{ $cat['subtitle'] }}</span>
                                <h3 class="text-3xl font-extrabold text-white mt-1">{{ $cat['title'] }}</h3>
                            </div>
                            ...
                            @foreach($cat['skills'] as $skill)
                                <div class="flex flex-col items-center justify-between p-6 bg-slate-900/30 border border-slate-800/60 rounded-2xl hover:border-slate-700/50 hover:bg-slate-900/50 transition-all duration-300 group/meter relative overflow-hidden">
                                    ...
                                    <div class="relative w-28 h-28 flex items-center justify-center mb-4">
                                        ...
                                        <div class="absolute inset-2 rounded-full opacity-0 group-hover/meter:opacity-10 transition-opacity duration-300" style="box-shadow: 0 0 30px {{ $cat['accent'] }}"></div>
                                        
                                        <svg class="w-full h-full transform -rotate-90">
                                            <!-- Track Circle -->
                                            <circle cx="56" cy="56" r="46" stroke="#1e293b" stroke-width="4px" fill="transparent" />
                                            <!-- Value Circle -->
                                            <circle cx="56" cy="56" r="46" stroke="{{ $cat['accent'] }}" stroke-dasharray="289" stroke-dashoffset="{{ 289 - (289 * $skill['level']) / 100 }}" stroke-width="5px" fill="transparent" class="transition-[stroke-dashoffset] duration-1000 ease-out" />
                                        </svg>
                                        <!-- Center Text -->
                                        <div class="absolute flex flex-col items-center justify-center">
                                            <span class="text-2xl font-black text-white font-mono leading-none">{{ $skill['level'] }}</span>
                                    ...
                                    <div class="text-center w-full mt-2">
                                        <h4 class="text-base font-bold text-white group-hover/meter:scale-105 transition-transform duration-300">{{ $skill['name'] }}</h4>
                                        <p class="text-[11px] text-slate-500 mt-1.5 leading-relaxed font-sans line-clamp-2">{{ $skill['desc'] }}</p>
                                    </div>
                                </div>
                            @endforeach
                        </div>
